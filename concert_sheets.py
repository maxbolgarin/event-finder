...
Auth: reads the FULL service-account JSON from GOOGLE_SA_JSON (environment
variable, or .env in the project root for local runs). Nothing sensitive is
committed to git.
Target sheet: SHEET_ID (env or .env; the long id in the sheet URL:
...
_HEADER_CELLS = {"artist", "artists", "festival", "festivals", "name"}

_ENV_VARS = ("GOOGLE_SA_JSON", "SHEET_ID")
_DOTENV_PATH = Path(__file__).resolve().parent / ".env"


def _load_env():
    if all(os.environ.get(k) for k in _ENV_VARS):
        return
...
def _client():
    ...
    if not raw:
        sys.exit("error: GOOGLE_SA_JSON is not set (env or .env)")
    try:
        info = json.loads(raw)
...
def _open():
    ...
    if not sheet_id:
        sys.exit("error: SHEET_ID is not set (env or .env)")
...
def _read_names(ss, tab, required):
    ...
    if names and names[0].lower() in _HEADER_CELLS:
        names = names[1:]
    return names
...
def _existing_keys(sched_ws):
    keys = set()
    for r in sched_ws.get_all_values()[1:]:  # skip header
